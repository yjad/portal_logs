...
def portal_projects():
    ...
    st.subheader("Reservation Portal Stats as of "+ logs.get_last_proj_rep_date())
...
def open_projects():
    ...
    df = db.query_to_pd(sql).set_index('project_id').drop(columns='index')
    
    st.subheader("Status of Open Projects "+ logs.get_last_proj_rep_date())
    st.dataframe(df.T)
